...
function MiniChallenge({data}:{data:ChallengeData}){const [choice,setChoice]=useState<number|null>(null);return <article className="panel challenge"><p className="eyebrow">Desafio</p><h2>{data.question}</h2><div className="answer-grid">{data.options.map((option,index)=><button key={option} disabled={choice!==null} className={choice===null?'':index===data.answer?'correct':choice===index?'wrong':''} onClick={()=>setChoice(index)}>{option}</button>)}</div>{choice!==null&&<div className={choice===data.answer?'feedback correct':'feedback'}><strong>{choice===data.answer?'Muito bem!':'Vamos rever.'}</strong> {data.explanation}</div>}<button className="primary-button" onClick={()=>setChoice(null)}>Tentar novamente</button></article>}
