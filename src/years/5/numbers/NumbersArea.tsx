...
type Tab =
  "decimal" | "fractions" | "operations" | "combinations" | "percentage";
const tabs = [
  ...
  { id: "combinations", label: "Fábrica de combinações", skills: "EF05MA09" },
  { id: "percentage", label: "Porcentagens", skills: "EF05MA06" },
] as const;
export function NumbersArea() {
  const [tab, setTab] = useState<Tab>("decimal");
  return (
    <section className="lab-page">
      <LabTabs tabs={[...tabs]} active={tab} onChange={setTab} />
      {tab === "decimal" && <PlaceValueActivity />}
      {tab === "fractions" && <FractionLineActivity />}
      {tab === "operations" && <CalculationWorkshopActivity />}
      {tab === "combinations" && <CombinationFactoryActivity />}
      {tab === "percentage" && <PercentageActivity />}
    </section>
  );
}
